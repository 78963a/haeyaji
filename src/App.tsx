...
export default function App() {
  ...
  // 1. 쌓인 일 (Unstarted tasks, status is pending/active, progress is zero)
  const unstartedTasks = tasks.filter(t => 
    (t.status === 'pending' || t.status === 'active') && 
    t.timeSpent === 0 && 
    (!t.actionLogs || t.actionLogs.length === 0) && 
    t.subtasks.every(st => !st.completed)
  );

  // 2. 해결중 (In-progress: some action logs, subtasks completed, or timeSpent > 0)
    ...
    <div className="bg-[#F4F4F1] min-h-screen text-[#1A1A1A] font-sans antialiased selection:bg-[#FF4D00] selection:text-white pb-28">
          ...
          >
            ...
            {activeView === 'active' && activeTask && (
              <ActiveFocusView
                ...
                onCheerTask={cheerTask}
                onStartActivityLog={startActivityLog}
                onEndActivityLog={endActivityLog}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#F4F4F1] border-t-4 border-black px-4 py-2 shadow-[0_-4px_0_rgba(0,0,0,0.05)]">
        <div className="max-w-xl mx-auto flex items-center justify-around relative">
          ...
          <button
            onClick={() => {}}
            className="flex flex-col items-center gap-0.5 p-2 focus:outline-none text-[#1A1A1A]/40 cursor-default"
          >
            <Clock className="w-5.5 h-5.5 stroke-[2] text-[#1A1A1A]/30" />
                        ...
                        <div className="flex-1 space-y-1.5">
                          <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                            <span className="bg-orange-100 text-[#FF4D00] text-[9px] font-black border border-[#FF4D00] px-1.5 py-0.5">
                              {t.timeSpent > 0 ? `${Math.round(t.timeSpent / 60)}분 전투` : '진행 개시됨'}
                            </span>
                            {t.subtasks.length > 0 && (
                    ...
                    completedTasks.map(t => (
                      <div key={t.id} className="bg-green-50 border-3 border-black p-3.5 shadow-[3px_3px_0px_0px_#000] space-y-1">
                        ...
                        <div className="text-[10px] text-zinc-600 font-black flex items-center gap-1.5 pt-1">
                          <span>⏱️ 누적 투입: <span className="font-mono text-black font-black">{t.timeSpent > 0 ? `${Math.round(t.timeSpent / 60)}분 ${t.timeSpent % 60}초` : '기록지 없음'}</span></span>
                          {t.subtasks.length > 0 && (
                            <>
                              <span>|</span>
                              <span>🛠️ 단계: <span className="text-black font-black">{t.subtasks.length}단계 소탕</span></span>
                            </>
                          )}
                        </div>
                      </div>
