...
export function HistoryView({ tasks, onStartTask, onDeleteTask, onUpdateTask }: HistoryViewProps) {
  ...
  const stats = useMemo(() => {
    ...
    const totalTimeSpent = tasks.reduce((acc, curr) => acc + curr.timeSpent, 0);
        ...
        {activeTab === 'stats' && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white p-4 border-3 border-black shadow-[4px_4px_0px_0px_#000]">
                <span className="text-[10px] text-zinc-600 font-extrabold block uppercase tracking-wider">누적 집중 행동 시간 ⏱️</span>
                <span className="text-lg font-black text-[#FF4D00] block mt-1.5 leading-none">
                  {formatSeconds(stats.totalTimeSpent)}
                </span>
                <p className="text-[10px] text-zinc-500 mt-1 font-bold">집중 타이머 가동 시간</p>
              </div>
              ...
              completedTasks.map((task) => (
                ...
                >
                  <div className="space-y-1.5 flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-[11px] text-zinc-600 pt-1 font-bold">
                      <span className="flex items-center gap-1">
                        <Clock className="w-3.5 h-3.5 text-black stroke-[2.5]" />
                        누적 집투 시간: {formatSeconds(task.timeSpent)}
                      </span>
                      {task.subtasks.length > 0 && (
                        <span className="text-[#FF4D00]">
                          🔨 세부 액션 플랜 {task.subtasks.length}개 격파 완료
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5 flex-1">
                    ...
                    <div className="flex items-center gap-4 text-[11px] text-zinc-500 pt-1 font-bold">
                      <span className="flex items-center gap-1 text-black">
                        <Clock className="w-3.5 h-3.5 text-black stroke-[2.5]" />
                        사투를 벌였던 시간: {formatSeconds(task.timeSpent)}
                      </span>
                    </div>
                  </div>
